...
| `configChanged`  | `{ config: MarkStudioConfig }`                         | A `markstudio.*` setting changed (T-111). The host re-reads the full config (per document URI, so resource-scoped overrides win) and sends the new snapshot; the webview applies it live — e.g. toggling the CM6 line-number gutter via a `Compartment` — without a reload. |
