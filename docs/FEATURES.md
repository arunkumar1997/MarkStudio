...
| Wiki links | Shipped | Wiki-style links (`[[note]]`, `[[note|alias]]`, `[[note#heading]]`) render as styled links in the preview via a dependency-free markdown-it inline rule; toggleable via `markstudio.preview.wikiLinks` (default on) and degrades to literal text when off; resolution to real files is deferred to Phase 4 (T-3.4, ADR-0018). |
