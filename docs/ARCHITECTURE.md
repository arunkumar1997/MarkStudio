...
│   ├── parseMarkdownTargets.ts       # pure Markdown-link target extractor (T-4.1a; no `vscode`/DOM)
│   ├── linkIndex.ts                  # pure reverse index + basename resolver + per-build heading-line cache (no `vscode`/fs)
...
| `parseWikiTargets` / `parseMarkdownTargets` / `linkIndex` | **Pure** wiki-link **and** Markdown-link target extraction + reverse-index/basename resolver (T-4.1, T-4.1a) + heading-line promotion via a per-build cache (T-4.1c, ADR-0024) feeding the backlinks panel and in-preview navigation via `resolveForward` (T-4.1b). `Backlink.kind?` distinguishes `"wiki"` from `"markdown"`; `Backlink.targetLine?: number \| null` carries the resolved heading line (`null` = lookup ran + missed) | Import `vscode`/DOM; touch the file system directly; refactor `NoteLink` (widen only — ADR-0024); resolve Markdown links by basename (explicit-path only — ADR-0024) |
...
| `LinkIndexService` | Async, batched workspace scan + `FileSystemWatcher` + debounced incremental rebuild; fire `onDidChangeIndex`; resolve a clicked **or hovered** target to URIs via `resolveTarget` (T-4.1, T-4.1b, M4.2, ADR-0020/0021/0022). Runs **both** wiki + Markdown extractors per file and threads the source text through `ParsedNote.text` so the index can resolve heading anchors at build time (T-4.1a + T-4.1c, ADR-0024). **One instance**, owned by `extension.ts`, injected into both the backlinks registration and the editor provider | Block activation; re-scan everything on each change; be instantiated more than once |
