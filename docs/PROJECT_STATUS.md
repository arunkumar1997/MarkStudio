...
* **Overall completion (qualitative):** Phase 0: 100%. Phase 1: 100%. Phase 2: 100%. Phase 3: 100%. **Phase 4: M4.1 + T-4.1b + M4.2 merged to `main`** (M4.4 graph view remains; M4.3 transclusion was dropped from scope).
* **Most recently merged:** **Bugfix — wiki-link clicks, Backlinks rows, AND standard markdown links now open `.md` targets in MarkStudio, not the built-in text editor (PR #4, `--no-ff` merge `dee909e`; ADR-0021 amendment + extension).** The first two commits on this branch (`dc8cda2` + `0b11632`, already pushed to PR #4) replaced `showTextDocument` with `vscode.openWith` for both wiki-link clicks and the Backlinks open command, and revealed the line through a **pending-reveal handshake** (a `controllersByUri` map + a new pure `PendingReveals` registry, `src/editor/pendingReveals.ts`). The uncommitted extension on top of that brings the same fix to **standard markdown links** in the preview (`[Architecture Overview](./ARCHITECTURE.md)`, `[doc](/docs/x.md)`, with or without a `#heading` fragment): a second delegated `click` listener (`registerMarkdownLinkClicks`, `src/webview/preview/markdownLinkClick.ts`) classifies the href through two pure helpers (`isExternalHref`, `parseLocalMarkdownHref`) and posts a new typed **`openMarkdownLink { href, target, heading }`** webview → host message (boundary-guarded in `messages.ts`); the host (`MarkStudioEditorProvider.openMarkdownLink`) resolves the path with **plain URI math** — *not* the workspace link index — (`/` → workspace-absolute against the source's workspace folder, falling back to the first folder; otherwise relative to the source's directory), then routes the target through the existing `openInMarkStudio` so the pending-reveal handshake is identical to the wiki-link / backlinks paths. External URLs, modifier-held clicks, fragment-only links, non-markdown extensions, and wiki-link anchors are left to the webview's default behaviour. Graceful degradation preserved (unresolved/failed open → transient status-bar `MarkStudio: no note found at "<href>"` / `could not open note at "<href>"`, never a throw). `activate()` now returns a tiny `MarkStudioExtensionApi { provider }` purely so the Extension Host navigation tests can drive the host handler directly — not a contributed extension API. **No new dependency, setting, or command.** Gate green: **199 unit + 65 integration + 9 exthost = 273**. Awaiting Producer review/merge.
* **Last updated:** 2026-06-30 by Remy (Producer) — PR #4 merged to `main` (wiki-link + Backlinks + standard markdown-link open-in-MarkStudio fix)
* **Last commit on `main`:** `dee909e` *(`--no-ff` merge of `fix/wikilink-open-in-markstudio` / PR #4 — ADR-0021 amendment + standard markdown-link extension). M4.2 (PR #3) was merged via `8bf1a86`; T-4.1b (PR #1) via `011901e`; M4.1 (T-4.1) via `79369f2`.*

---

## 2. Current Focus

* **Active initiative:** **Phase 4 — Knowledge Management.** M4.1 (Backlinks panel, T-4.1), T-4.1b (in-preview wiki-link navigation), and **M4.2 (Hover preview for links)** are all merged to `main`. The next milestone is **M4.4 — Graph view** (M4.3 — transclusion was dropped from scope).
...
* **Target outcome:** M4.2 merged ✅; **next: M4.4 — Graph view**. See [TODO.md](TODO.md) and [AGENT_HANDOFF.md](AGENT_HANDOFF.md) §10.
...
| ---- | ----- | ----- | ----- |
| **M4.2 — Hover preview for links** | Implemented on `feature/sprint-4`; pipeline green; pending QA F5 + merge | Sage + Nova | Next roadmap milestone after merge is M4.4 — Graph view (M4.3 transclusion was dropped from scope) |
