...
* **Current milestone:** **M4.2 — Hover preview for links: implemented on `feature/sprint-4`** (pending QA F5 + Producer merge). Hovering a rendered wiki-link in the preview now shows, after a short dwell (~300 ms), a floating **hover card** previewing the target — `[[note]]` the top of the note, `[[note#heading]]` the section under that heading — the read-side counterpart to T-4.1b's click-to-open. A single **delegated** `pointerover`/`pointerout` pair on the persistent preview pane posts a new typed **`requestLinkPreview`** webview → host message; the host resolves through the **shared M4.1 resolver** (`LinkIndexService.resolveTarget`, open-first), reads a capped excerpt via a new pure `src/links/linkExcerpt.ts` (heading-section slice or top of note, ≤ 60 lines / ≤ 2,000 chars), and replies with a new **`linkPreviewContent`** message carrying **Markdown text, not HTML** — the webview renders it with the **existing** `PreviewRenderer` (`html: false` preserved) into a card themed with VS Code hover-widget tokens. Stale replies are dropped; an unresolved/failed read shows a quiet "No note found" card. Producer policy: open-first on ambiguity, **no new setting** (gated by `markstudio.preview.wikiLinks`), static snapshot. **No new dependency, setting, or command.** (ADR-0022.) T-4.1b and M4.1 remain merged to `main` (`011901e` / `79369f2`).
* **Overall completion (qualitative):** Phase 0: 100%. Phase 1: 100%. Phase 2: 100%. Phase 3: 100%. **Phase 4: M4.1 + T-4.1b merged; M4.2 done on `feature/sprint-4`** (M4.3 transclusion, M4.4 graph view remain).
* **Last updated:** 2026-06-30 by the Dev Team (Sage + Nova) — M4.2 implemented on `feature/sprint-4`
* **Last commit on `main`:** `011901e` *(`--no-ff` merge of `feature/sprint-3` / PR #1 — T-4.1b in-preview wiki-link navigation). M4.1 (T-4.1) was merged earlier via `79369f2`. M4.2 work lives on `feature/sprint-4` (off `a6d48af`), not yet merged.*

---

## 2. Current Focus

* **Active initiative:** **Phase 4 — Knowledge Management.** M4.1 (Backlinks panel, T-4.1) and T-4.1b (in-preview wiki-link navigation) are merged to `main`. **M4.2 — Hover preview for links** is implemented on `feature/sprint-4` (pipeline green; pending QA F5 + merge). The next milestone is **M4.3 — Embedded notes / transclusion**.
...
* **Target outcome:** M4.2 merged after QA sign-off + the F5 matrix; then M4.3. See [TODO.md](TODO.md) and [AGENT_HANDOFF.md](AGENT_HANDOFF.md) §10.
