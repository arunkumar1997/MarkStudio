...
* **Active initiative:** **Phase 5 — Authoring Workflows is in progress.** Sprint 7 shipped **M5.1 — Templates** and **M5.3 — Daily notes** (one engine, two consumers) on `feature/sprint-7`, pending PR + `--no-ff` merge. Next is **M5.2 — Snippets** (Sprint 8), which layers on the `${N}` placeholders the M5.1 expander already passes through verbatim; **M5.4 — Workspace note features** is deferred. See [AGENT_HANDOFF.md](AGENT_HANDOFF.md) §10.
* **Owner (this sprint):** the dev team (Nova — UI/QuickPick, Sage — host service + pure core, Milo — visual polish); tests Ivy; QA: Ivy + Producer F5
* **Started:** Sprint 7 executed 2026-06-30
* **Target outcome:** M5.1 + M5.3 ✅ shipped on `feature/sprint-7` (461 tests green; every prior assertion intact); pending PR + `--no-ff` merge. Next: **Sprint 8 — M5.2 Snippets** — see [TODO.md](TODO.md) and [AGENT_HANDOFF.md](AGENT_HANDOFF.md) §10.
...
| **Templates + Daily notes (Phase 5, M5.1 + M5.3)** — three commands (`MarkStudio: New Note from Template` → native QuickPick → title `InputBox` → create + open in MarkStudio; `MarkStudio: Open Today's Note` → one-key create-or-open; `MarkStudio: Create Example Template` → opt-in starter `daily` template) over four **pure** modules (`frontMatterParser` in-tree `---` reader, `dateFormatter` over `Intl.DateTimeFormat`, `variableExpander` closed allowlist + `{{cursor}}` + `slugify` with snippet `${N}` passing through verbatim, `templateResolver` workspace+user merge by basename) and one host `TemplateService` (two-root async scan, two `FileSystemWatcher`s, 250 ms debounce, `onDidChangeTemplates`, **two-pass** create, **never overwrite**, opens via `provider.openInMarkStudio`). Five settings (`markstudio.templates.{workspaceFolder,userFolder}`, `markstudio.dailyNotes.{template,folder,dateFormat}`). **Zero new runtime dependencies**, no webview/protocol/esbuild change, no new keybinding or view, no auto-bootstrap (ADR-0025) | 5 | Unreleased (M5.1 + M5.3, `feature/sprint-7` pending merge) |
...
| ---- | ----- | ----- | ----- |
| **M5.1 — Templates + M5.3 — Daily notes** | Implemented on `feature/sprint-7` (Phases A → E); 461 tests green; human F5 outstanding (Producer); pending PR + `--no-ff` merge | Sage + Nova + Milo | After merge: **Sprint 8 — M5.2 Snippets** (reuses the `${N}` placeholders the M5.1 expander passes through verbatim + the CM6 snippet session). M5.4 (workspace note features) deferred |
