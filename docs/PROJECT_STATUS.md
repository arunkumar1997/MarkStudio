...
* **Current milestone:** **M4.4 — Graph view: SHIPPED on `feature/sprint-5`, pending PR + `--no-ff` merge to `main`** (Sprint 5, automated **335 PASS** + human F5 verified + perf trace **median 100 fps**). A new `MarkStudio: Show Graph` command (plus an editor title-bar action with the `$(type-hierarchy)` icon while a MarkStudio editor is active) opens an interactive workspace-wide graph of every note (circle) and every wiki-link (edge): pan, wheel-zoom around the cursor, drag-pin, hover → 1-hop neighbour highlight, click to open the target **in MarkStudio** (PR #4 handshake), Escape resets the view. The host (`src/graph/GraphService.ts`) owns a single `vscode.WebviewPanel` (`retainContextWhenHidden: true`); it posts a typed **`graphData { nodes, edges, currentPath }`** message built by a new **pure** `src/graph/graphModel.ts` over a new **pure** `LinkIndex.allEdges()` getter (deterministic ASCII-codepoint dedup with weight summed in the same build pass) — whenever the link index changes (debounced 250 ms) or the active MarkStudio document changes. Click-to-open posts **`openGraphNode { path }`** and the host routes through `provider.openInMarkStudio(uri, 0)` — the PR #4 pending-reveal handshake, never `showTextDocument`. The webview ships as a **lazy 4th esbuild bundle** `dist/graph.js` (**19.3 kB**, mirrors the Mermaid pattern from ADR-0016) and contains a hand-rolled Fruchterman–Reingold simulation (deterministic FNV-1a `seedPosition`) + Canvas2D body + DOM labels + RAF loop, with merge-by-path on `graphData` so known nodes keep positions. Every frame samples live `--vscode-*` tokens via `getComputedStyle` so theme switches are reactive without host wiring. Producer policy: **zero new runtime dependencies** (d3-force / cytoscape.js / vis-network all rejected — ADR-0023), **wiki-link only** in v1 (T-4.1a / T-4.1c deferred), **no new setting**, click-to-open **must** route through `provider.openInMarkStudio` (non-negotiable). M4.2 + T-4.1b + M4.1 remain merged to `main` (`8bf1a86` / `011901e` / `79369f2`).
* **Overall completion (qualitative):** Phase 0: 100%. Phase 1: 100%. Phase 2: 100%. Phase 3: 100%. **Phase 4: 100%** (M4.1 + T-4.1b + M4.2 merged to `main`; M4.4 shipped on `feature/sprint-5`, pending merge; M4.3 transclusion dropped from scope).
* **Most recently shipped:** **M4.4 — Graph view (Sprint 5, `feature/sprint-5`, 6 commits, pending PR + merge).** See `docs/sprint-5/done.md` and `docs/qa/sprint-5-signoff.md`. Gate green: **257 unit + 65 integration + 13 exthost = 335, 0 fail**. Perf: median 100 fps, p95 50 fps, 1.9 % slow frames. New ADR-0023.
* **Last updated:** 2026-06-30 by Remy (Producer) — Sprint 5 M4.4 graph view shipped on `feature/sprint-5` (Phases A → E complete)
* **Last commit on `feature/sprint-5`:** `d1c7069` *(test(graph): forceSimulation + boundary guard + exthost coverage — Phase D, M4.4). Branch is 6 commits ahead of `main` (`dee909e`).*

---

## 2. Current Focus

* **Active initiative:** **Phase 4 — Knowledge Management is closed.** M4.1 (Backlinks panel, T-4.1), T-4.1b (in-preview wiki-link navigation), M4.2 (Hover preview), and **M4.4 (Graph view)** are all shipped (M4.4 pending merge from `feature/sprint-5`). M4.3 — transclusion was dropped from scope. The next focus is **Phase 5 — Authoring Workflows** (templates / snippets / daily notes / workspace note features); see [AGENT_HANDOFF.md](AGENT_HANDOFF.md) §10.
* **Owner (this sprint):** Sage (host model + service + messaging) + Nova (canvas webview + simulation + interactions); tests Ivy; QA: Ivy
* **Started:** Sprint 5 executed 2026-06-30
* **Target outcome:** M4.4 ✅ shipped on `feature/sprint-5` (335 tests green, F5 verified, perf budget exceeded); pending PR + `--no-ff` merge. Next: **Phase 5 kickoff** — see [TODO.md](TODO.md) and [AGENT_HANDOFF.md](AGENT_HANDOFF.md) §10.
