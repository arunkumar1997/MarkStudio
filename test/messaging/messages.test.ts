...
describe("isHostToWebviewMessage", () => {
    ...
    describe("init", () => {
        it("accepts a well-formed init with config", () => {
            assert.equal(
                isHostToWebviewMessage({ type: "init", text: "# Hi", config: VALID_CONFIG }),
                true
            );
        });
    ...
    describe("configChanged", () => {
        ...
        it("rejects configChanged with a malformed config", () => {
            ...
            assert.equal(
                isHostToWebviewMessage({ type: "configChanged" }),
                false
            );
        });
    });
    ...
    describe("setContent", () => {
        it("accepts text and rejects non-string text", () => {
            assert.equal(isHostToWebviewMessage({ type: "setContent", text: "x" }), true);
            assert.equal(isHostToWebviewMessage({ type: "setContent", text: 1 }), false);
        });
    });
    ...
    describe("setLayoutMode", () => {
        it("accepts each valid mode", () => {
            for (const mode of ["split", "editor-only", "preview-only"]) {
                assert.equal(isHostToWebviewMessage({ type: "setLayoutMode", mode }), true);
            }
        });
    ...
    describe("error", () => {
        it("accepts a string message and rejects otherwise", () => {
            assert.equal(isHostToWebviewMessage({ type: "error", message: "boom" }), true);
            assert.equal(isHostToWebviewMessage({ type: "error", message: 5 }), false);
        });
    });
});
...
describe("isWebviewToHostMessage", () => {
    ...
    describe("layoutModeChanged", () => {
        it("accepts a valid mode and rejects an invalid one", () => {
            assert.equal(
                isWebviewToHostMessage({ type: "layoutModeChanged", mode: "editor-only" }),
                true
            );
            assert.equal(
    ...
    describe("error", () => {
        it("accepts a string message", () => {
            assert.equal(isWebviewToHostMessage({ type: "error", message: "x" }), true);
