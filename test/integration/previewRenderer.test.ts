...
import { createPreviewRenderer, type PreviewRenderer } from "../../src/webview/preview/PreviewRenderer";
...
describe("createPreviewRenderer — incremental DOM patching", () => {
    ...
    it("preserves unchanged block nodes and replaces only the edited block", async () => {
        ...
        assert.equal(after[2], before[2], "trailing paragraph node should be reused");
        // The edited block is a new node with the new text.
        assert.notEqual(after[1], before[1], "edited paragraph node should be replaced");
