...
import { createEditor, type ChangeBatch } from "../../src/webview/editor/createEditor";
...
describe("createEditor — host/local edit seam", () => {
    ...
    function build(initialText: string, initialCursor?: { anchor: number; head: number }) {
        return createEditor({
            parent: container,
            initialText,
    ...
    it("applies host content without echoing it back as a local edit", () => {
        ...
        assert.equal(batches.length, 0, "host content must not round-trip as an edit");
